...
- `packages/shared-types` 承载跨包共享的纯类型契约（零运行时依赖）
- `packages/shared-utils` 承载通用工具与 HTTP 客户端接口
...
> **注意**：`packages/shared` 已废弃（ADR-010），其原有职责已收敛到 `shared-types`、`shared-utils`、`shared-i18n` 等源头包。
